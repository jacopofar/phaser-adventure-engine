...
export class ChunkManager {
  ...
  constructor(loadThreshold: integer = 2000, unloadThreshold: integer = 4000, reactionDistance: integer = 32) {
    if (unloadThreshold <= loadThreshold) {
      throw Error("Unload threshold should be higher than load one or it will keep loading/unloading the edges");
    }
  ...
  async handleNewPosition(targetScene: Phaser.Scene, x: integer, y: integer) {
    ...
    for(let offx = -1; offx <= 1; offx++) {
      for(let offy = -1; offy <= 1; offy++) {
        ...
        const minChunkX = this.world.offsetX + (curChunkX + offx) * this.world.multiplierX;
        const maxChunkX = this.world.offsetX + (curChunkX + offx + 1) * this.world.multiplierX;

        const minChunkY = this.world.offsetY + (curChunkY + offy) * this.world.multiplierY;
        const maxChunkY = this.world.offsetY + (curChunkY + offy + 1) * this.world.multiplierY;


        if (this.loadedChunks.hasOwnProperty(thisChunk)) {
          // loaded, check for unload condition: is it completely outside the unload area?
          if (!(maxunloadX >= minChunkX && minunloadX <= maxChunkX && minunloadY <= maxChunkY && maxunloadY >= minChunkY)) {
          ...
          if (maxloadX >= minChunkX && minloadX <= maxChunkX && minloadY <= maxChunkY && maxloadY >= minChunkY) {
            const newChunk = new Chunk();
            this.loadedChunks[thisChunk] = newChunk;
            await newChunk.loadMap(
              targetScene,
              [this.world.pathPre, `${curChunkX + offx}`, this.world.pathMiddle, `${curChunkY + offy}`, this.world.pathPost].join(''),
              minChunkX,
              minChunkY
            )
          }
